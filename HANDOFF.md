...
- **재고 자동 차감**: 주문 상태 '배송완료' 시 차감, 되돌리기 시 복원

## 재고 흐름 정리

[재고가 늘어나는 경우]
- 발주 입고완료: 거래처에서 물건이 들어오면 재고 증가
- 수동 입고/조정: 관리자가 직접 재고 수량 추가
- 배송완료 되돌리기: 배송완료를 이전 상태로 되돌리면 재고 복원
- 반품/취소 완료: 반품이 완료되면 재고 자동 복원
- 배송완료 주문 삭제: 배송완료된 주문을 삭제하면 재고 복원

[재고가 줄어드는 경우]
- 배송완료 처리: 주문이 배송완료되면 재고 차감
- 수동 조정: 관리자가 직접 재고 수량 감소

[재고 변동 없음 - 배정만]
- 주문 생성 (입금확인): 재고 변동 없음
- 결제완료: 재고 변동 없음, 입고된 물량을 주문에 배정만
- 배송준비 자동전환: 재고 변동 없음, 배정 수량만 표시
- 결제취소: 배정된 수량만 해제, 재고 변동 없음
